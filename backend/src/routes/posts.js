...
router.post('/', async (req, res) => {
    await PostsService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  const payload = await PostsDBApi.findAll(
    req.query,
  );
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await PostsDBApi.findBy(
    { id: req.params.id },
  );
